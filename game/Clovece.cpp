...
void Clovece::doTurn() {
    ...
    if (hasMoves(roll)) {
        ...
        int f = getFigureToMove(true, roll);
        if (f == -1) {
            return;
        }
...
int Clovece::getFigureToMove(bool bot, int roll) {
    int f = 0;
    if (bot) {
        for (int i = 0; i < 4; i++) {
...
bool Clovece::moveFigure(Move move) {
    return moveFigure(getFigure(move), move.getRolled(), false);
}
